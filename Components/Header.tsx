...
export default function Header() {
    ...
    return (
        <header className="z-[999] relative w-full">
            <motion.div
                className={clsx(
                    "fixed top-[34px] sm:top-[3rem] right-[1.85rem] sm:right-20 lg:h-[4.5rem] rounded-2xl border border-white border-opacity-40 bg-secondary lg:w-[29rem] transition-all duration-400 ease-in-out dark:border-tertiary dark:bg-darkBlack dark:text-white",
                    { "h-[350px]": isMobileNavActive }
                )}
                initial={{ y: -100, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
            >
                <div className="lg:hidden w-full h-full flex flex-col justify-center items-center rounded-lg p-[0.2rem] ">
